...
**Model policy:** choose freely per task. Do not force a single Claude model. Use a full model ID when targeting a specific release, or omit `model` to use Claude Code's configured default.

| Model ID             | Model             | Typical effort |
| -------------------- | ----------------- | -------------- |
| `claude-opus-4-8`    | Claude Opus 4.8   | `high`         |
| `claude-opus-5`      | Claude Opus 5     | `high`         |
| `claude-sonnet-5`    | Claude Sonnet 5   | `medium`/`high` |
| `opus` / `sonnet`    | Latest CLI alias  | task-dependent |

Other full model IDs and aliases supported by the installed Claude Code are allowed; this list is not a whitelist.
