 ...
 * - Popup UI: arrow keys, number keys, or a mouse click to pick; Enter confirms
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      const showQuestion = (uiSignal: AbortSignal) =>
        ctx.ui.custom<SelectionResult>((tui, theme, _kb, done) => {
          ...
          let optionHitRows: Array<{
            start: number;
            end: number;
            index: number;
          }> = [];
          let mouseResumeTimer: ReturnType<typeof setTimeout> | undefined;
          let mouseEnabled = false;

          let settled = false;

          function enableMouse() {
            if (mouseEnabled || settled) return;
            mouseEnabled = true;
          ...
          function handleMouse(data: string): boolean {
            ...
            const screenRow = Number(match[3]) - 1;

            if ((button & 64) !== 0) {
              disableMouse();
              if (mouseResumeTimer) clearTimeout(mouseResumeTimer);
              mouseResumeTimer = setTimeout(() => {
                mouseResumeTimer = undefined;
                enableMouse();
              }, 750);
              return true;
            ...
            const isLeftPress = match[4] === "M" && (button & 35) === 0;
            if (!isLeftPress || !cachedLines) return true;
            ...
            const overlayTop = 1 + Math.floor((terminalRows - 2 - overlayHeight) / 2);
            const localRow = screenRow - overlayTop;
            const hit = optionHitRows.find(
              ({ start, end }) => localRow >= start && localRow <= end,
            );

            if (hit) {
              optionIndex = hit.index;
              selectOption(hit.index);
            }
            return true;
          }
          ...
          function render(width: number): string[] {
            ...
            if (editMode) {
              ...
              add(
                theme.fg(
                  "dim",
                  ` Click or ↑↓/1-${allOptions.length} select • Enter confirm • wheel scroll • Esc dismiss`,
                ),
              );
            }
      ...
      let uiExit;
      try {
        uiExit = await Effect.runPromiseExit(
