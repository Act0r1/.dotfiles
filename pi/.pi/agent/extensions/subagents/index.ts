...
interface PersistentSubagentState {
  runtime?: SubagentRuntime;
  managerPromise?: Promise<SubagentManagerShape>;
  resultDelivery: ReturnType<
    typeof createDeferredResultDelivery<SubagentSnapshot>
  >;
  pendingSettled: Array<{
    snap: SubagentSnapshot;
    consumed: boolean;
  }>;
  onSettled?: (snap: SubagentSnapshot, consumed: boolean) => void;
}

const SUBAGENT_STATES_KEY = Symbol.for("pi.extensions.subagents.state.v1");
const subagentGlobal = globalThis as typeof globalThis &
  Record<symbol, unknown>;

function subagentStates() {
  let states = subagentGlobal[SUBAGENT_STATES_KEY] as
    | Map<string, PersistentSubagentState>
    | undefined;
  if (!states) {
    states = new Map();
    subagentGlobal[SUBAGENT_STATES_KEY] = states;
  }
  return states;
}

function persistentSubagentState(sessionId: string) {
  let state = subagentStates().get(sessionId);
  if (!state) {
    state = {
      resultDelivery: createDeferredResultDelivery<SubagentSnapshot>(),
      pendingSettled: [],
    };
    subagentStates().set(sessionId, state);
  }
  return state;
}

export default function (pi: ExtensionAPI) {
  let state: PersistentSubagentState | undefined;
  let sessionId: string | undefined;
  let sessionContext: ExtensionContext | undefined;
  let ui: ExtensionUIContext | undefined;
  let unsubStatus: (() => void) | undefined;
  let unsubOpenEvent: (() => void) | undefined;
  let boundManager: SubagentManagerShape | undefined;

  const getState = () => {
    if (!state) throw new Error("Subagent session is not initialized.");
    return state;
  };

  const getRuntime = () => {
    const current = getState();
    return (current.runtime ??= createSubagentRuntime());
  };
  ...
  const updateStatus = (manager: SubagentManagerShape) => {
    ...
    if (subs.length === 0) {
      setActivityStatus(ui, "subagents");
      return;
    }
    const running = subs.filter((snap) => snap.status === "running").length;
    const failed = subs.filter((snap) => snap.status === "error").length;
    const done = subs.length - running - failed;
    setActivityStatus(ui, "subagents", { running, done, failed });
  };

  const bindManager = (manager: SubagentManagerShape) => {
    if (boundManager === manager && unsubStatus) {
  ...
  const getManager = async () => {
    const current = getState();
    current.managerPromise ??= getRuntime()
      .runPromise(SubagentManager)
      .then((manager) => {
        manager.view.setOnSettled((snap, consumed) => {
          const copy = { ...snap, meta: { ...snap.meta } };
          if (current.onSettled) {
            try {
              current.onSettled(copy, consumed);
            } catch {
              current.pendingSettled.push({ snap: copy, consumed });
            }
          } else {
            current.pendingSettled.push({ snap: copy, consumed });
          }
    ...
    const manager = await current.managerPromise;
    if (state === current && sessionContext) bindManager(manager);
    return manager;
  };
  ...
  const flushResults = () => {
    if (!state) return;
    const pending = state.resultDelivery.drain();
    for (let index = 0; index < pending.length; index++) {
      try {
        deliverResult(pending[index]);
      } catch {
        for (const snap of pending.slice(index)) {
          state.resultDelivery.defer(snap);
        }
        return;
      }
    }
  ...
  const onSettled = (snap: SubagentSnapshot, consumed: boolean) => {
    ...
    if (snap.origin === "btw") {
      deliverBtwResult(snap);
      return;
    }
    if (consumed) {
      state.resultDelivery.consume([snap.id]);
      return;
    }
    state.resultDelivery.defer(snap);
  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    state = persistentSubagentState(sessionId);
    sessionContext = ctx;
    if (ctx.hasUI) ui = ctx.ui;
    state.onSettled = onSettled;
    const pending = state.pendingSettled.splice(0);
    for (const item of pending) {
      try {
        onSettled(item.snap, item.consumed);
      } catch {
        state.pendingSettled.push(item);
      }
    }
    if (ctx.isIdle()) flushResults();
    if (state.managerPromise) void getManager().catch(() => {});
  });
  ...
  pi.on("session_shutdown", async (event) => {
    ...
    unsubOpenEvent?.();
    unsubOpenEvent = undefined;
    boundManager = undefined;
    if (ui) setActivityStatus(ui, "subagents");
    ui = undefined;
    if (!closingState) return;
    if (closingState.onSettled === onSettled) {
      closingState.onSettled = undefined;
    }
    state = undefined;
    if (event.reason === "reload") return;
    closingState.resultDelivery.clear();
    closingState.pendingSettled.length = 0;
    const closing = closingState.runtime;
    closingState.runtime = undefined;
    closingState.managerPromise = undefined;
    if (sessionId) subagentStates().delete(sessionId);
    sessionId = undefined;
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, onUpdate) {
      ...
      getState().resultDelivery.consume(ids);
  ...
  let subagentsUiOpen = false;

  const showSubagents = async (ctx: ExtensionContext) => {
    if (subagentsUiOpen) return;
    if (ctx.mode !== "tui") {
      if (ctx.hasUI)
        ctx.ui.notify(
    ...
    subagentsUiOpen = true;
    try {
      await openSubagentPicker(ctx, manager.view);
    } finally {
      subagentsUiOpen = false;
    }
  };

  unsubOpenEvent = pi.events.on("subagents:open", () => {
    if (sessionContext) void showSubagents(sessionContext);
  });
