...
class TakeoverView implements Component, Focusable {
  ...
  private totalTranscriptLines = 0;
  private lastTranscriptWidth = 0;
  ...
  render(width: number): string[] {
    ...
    const previousTotalLines = this.totalTranscriptLines;
    this.totalTranscriptLines = transcript.length;
    if (this.lastTranscriptWidth > 0 && width !== this.lastTranscriptWidth) {
      this.scrollOffset = 0;
    } else if (this.scrollOffset > 0) {
      this.scrollOffset = Math.max(
        0,
        this.scrollOffset + this.totalTranscriptLines - previousTotalLines,
      );
    }
