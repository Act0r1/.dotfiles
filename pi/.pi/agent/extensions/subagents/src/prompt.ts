...
export const SUBAGENT_SPAWN_PARAMETER_DESCRIPTIONS = {
  ...
  model:
    'Model hint, interpreted by the chosen harness. For Claude Code, choose any installed alias or full model id per task, for example "claude-opus-4-8", "claude-opus-5", "claude-sonnet-5", "opus", or "sonnet"; there is no fixed Claude model. For pi use "provider/model-id" or an unambiguous model id; for Codex use a model slug. Omit for that harness default (pi inherits the current model; Claude Code uses its configured default).',
  reasoningEffort:
