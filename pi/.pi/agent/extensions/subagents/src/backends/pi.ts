...
function resolvePiModel(
  ...
  if (!hint) {
    ...
    return find(inherited.provider, inherited.id);
  }
  ...
  if (inherited) {
    const found = find(inherited.provider, hint);
    if (found) return found;
  }
  const matches = models.filter((m) => m.id === hint);
...
const makePiSession = (
  ...
  Effect.gen(function* () {
    ...
    const model = yield* Effect.try({
      try: () =>
        resolvePiModel(models, task.model, task.parent.inheritedModel),
    ...
    const activeModel = (): Model<any> | undefined => {
      ...
      return (
        models.find(
          (model) =>
            model.provider === last.provider &&
            model.id === (last.responseModel ?? last.model),
        ) ?? sessionModel
      );
    };
