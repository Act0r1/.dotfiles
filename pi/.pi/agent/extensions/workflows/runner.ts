 ...
 *
 * Each pi-harness `agent()` call in a workflow script becomes one isolated
 * in-process AgentSession created here: in-memory session, normal trust-aware resources
...
export interface RunAgentOptions {
  ...
  modelCatalog: ReadonlyArray<WorkflowModel>;
  signal?: AbortSignal;
  ...
  const sync = () => {
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      const reportedModel = responseMatchesSession
        ? options.modelCatalog.find(
            (model) =>
              model.provider === msg.provider && model.id === reportedId,
          )
        : undefined;
      if (reportedModel) {
