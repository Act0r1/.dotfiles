...
export const WORKFLOW_TOOL_DESCRIPTION = [
  ...
  "• await agent(prompt, { label?, phase?, schema?, harness?, model?, provider?, effort? }) — run ONE subagent in an isolated context and wait for it. Always resolves to { ok, output, structured?, error? }. Check `ok` before using the result. `harness` defaults to `pi`; use `harness: 'claude'` for the Claude Agent SDK backend or `harness: 'codex'` for Codex CLI. For Claude, `model` accepts any Claude Code alias or full model id, including `claude-opus-4-8`, `claude-opus-5`, and `claude-sonnet-5`; choose freely per task or omit it for Claude Code's configured default. For pi, `model`/`provider` select a pi registry model. `effort` is off|minimal|low|medium|high|xhigh|max. JSON `schema` produces validated `structured` output on pi and Claude; Codex workflow agents currently require prose output. Children cannot recursively orchestrate or ask the user.",
...
export const WORKFLOW_PROMPT_GUIDELINES = [
  ...
  "For workflow Claude Code agents, set `harness: 'claude'` and choose the model freely per task; do not force every task onto one Claude model.",
];
