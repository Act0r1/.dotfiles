...
M.BASE_URL = "https://api.anthropic.com/v1/messages"
...
local function log(section, content)
  ...
  if f then
    f:write("\n" .. string.rep("=", 60) .. "\n")
    f:write(os.date("%Y-%m-%d %H:%M:%S") .. " [ANTHROPIC] " .. section .. "\n")
...
function M.new()
  ...
  function self:is_available()
    ...
    return cfg.api_keys.anthropic ~= nil and cfg.api_keys.anthropic ~= ""
  end
  ...
  function self:complete(context, callback, on_error)
    ...
    local body = {
      ...
      system = [[You are a code completion assistant. Output ONLY raw code to insert.
...
- When unsure what type/value to use, output NOTHING]],
      ...
      -- stop_sequences removed - API requires non-whitespace
    }

    http.post(M.BASE_URL, {
      headers = {
        ["Content-Type"] = "application/json",
        ["x-api-key"] = cfg.api_keys.anthropic,
      ...
      callback = function(response)
        ...
        if response.content and response.content[1] then
          completion = response.content[1].text or ""
        end
