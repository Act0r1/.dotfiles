...
RowLayout {
    ...
    signal workspaceNavigationRequested(int step)
    ...
    ToolbarTextField { // Search box
        ...
        Keys.onPressed: (event) => {
            if (!root.searchingText && event.modifiers === Qt.NoModifier
                    && (event.key === Qt.Key_Up || event.key === Qt.Key_Down)) {
                root.workspaceNavigationRequested(event.key === Qt.Key_Up ? -1 : 1)
                event.accepted = true
            } else if (actionModeView?.visible) {
