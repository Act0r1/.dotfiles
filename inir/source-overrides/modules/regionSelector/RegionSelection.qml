    ...
    function snip() {
        ...
        switch (root.action) {
            ...
            case RegionSelection.SnipAction.CharRecognition: {
                ...
                snipProc.command = ["/usr/bin/bash", "-c", `if ${cropInPlace} && _ocr_text="$('${ocrRunner}' '${StringUtils.shellSingleQuoteEscape(root.screenshotPath)}' '${StringUtils.shellSingleQuoteEscape(ocrLanguage)}')"; then if [[ -n "\${_ocr_text//[[:space:]]/}" ]]; then if printf '%s' "$_ocr_text" | '${clipboardCopy}' && printf '%s' "$_ocr_text" | '${clipboardCopy}' --primary; then /usr/bin/notify-send "Text recognized" "OCR text copied to clipboard" -a "OCR" -i edit-find -t 3000; else /usr/bin/notify-send "OCR failed" "Could not copy recognized text" -a "OCR" -i dialog-error -t 4000; fi; else /usr/bin/notify-send "No text recognized" "Try selecting the text more closely. Clipboard unchanged." -a "OCR" -i edit-find -t 4000; fi; else /usr/bin/notify-send "OCR failed" "Could not capture or recognize the selected text. Clipboard unchanged." -a "OCR" -i dialog-error -t 4000; fi; ${cleanup}`];
                break;
            }
