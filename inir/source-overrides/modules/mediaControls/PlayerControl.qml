...
Item {
    ...
    component CardDragArea: MouseArea {
        ...
        onPressed: mouse => {
            pressPosition = mapToItem(root.dragTarget.parent, mouse.x, mouse.y)
            startPosition = Qt.point(root.dragTarget.x, root.dragTarget.y)
            moving = false
        }
        onPositionChanged: mouse => {
            if (!pressed || !root.dragTarget) return
            const position = mapToItem(root.dragTarget.parent, mouse.x, mouse.y)
            const dx = position.x - pressPosition.x
            const dy = position.y - pressPosition.y
            if (!moving && Math.hypot(dx, dy) < Qt.styleHints.startDragDistance) return
            moving = true
            root.dragTarget.x = Math.max(0, Math.min(startPosition.x + dx,
                root.dragTarget.parent.width - root.dragTarget.width))
            root.dragTarget.y = Math.max(0, Math.min(startPosition.y + dy,
                root.dragTarget.parent.height - root.dragTarget.height))
        }
    }
