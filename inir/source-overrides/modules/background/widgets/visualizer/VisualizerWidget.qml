...
AbstractBackgroundWidget {
    ...
    visible: opacity > 0 && (GlobalStates.widgetEditMode
        || (root._active && cavaProcess.audioSignalActive
            && !(Audio.sink?.audio?.muted ?? false) && Audio.value > 0))

    readonly property bool _active: !GlobalStates.screenLocked
        && root.widgetOpacity > 0
