...
require_microphone() {
    if [[ "$AUDIO_MODE" == "microphone" || "$AUDIO_MODE" == "both" ]]; then
        local microphone_source
        microphone_source="$(resolve_microphone_device)"
        if [[ "$microphone_source" == *.monitor ]] || ! audio_source_exists "$microphone_source"; then
            notify-send "Recording cancelled" "The selected microphone is unavailable. Connect it and try again." -a 'Recorder' -u critical || true
            return 1
        fi
    fi
    return 0
...
prepare_audio_capture() {
    ...
    case "$AUDIO_MODE" in
        ...
        both)
            ...
            notify-send "Recording cancelled" "Could not capture system audio and microphone together. No video was recorded." -a 'Recorder' -u critical || true
            return 1
            ;;
    esac
    ...
    if ! audio_source_exists "$AUDIO_CAPTURE_DEVICE"; then
        if [[ "$AUDIO_MODE" == "microphone" ]]; then
            notify-send "Recording cancelled" "The selected microphone became unavailable. No video was recorded." -a 'Recorder' -u critical || true
            return 1
        fi
